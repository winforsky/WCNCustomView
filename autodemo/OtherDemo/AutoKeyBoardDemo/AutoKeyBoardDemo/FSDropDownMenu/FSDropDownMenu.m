...
- (instancetype)initWithOrigin:(CGPoint)origin andRowHeight:(CGFloat)rowHeight andHeight:(CGFloat)height {
    CGSize screenSize = [UIScreen mainScreen].bounds.size;
    self = [self initWithFrame:CGRectMake(origin.x, origin.y, screenSize.width, 0)];
    if (self) {
        _origin = origin;
        _show = NO;
        _height = height >(screenSize.height-origin.y)?(screenSize.height-origin.y):height;
        
        
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height, ScreenWidth, 0) style:UITableViewStylePlain];
...
- (void)animateTableViewShow:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth, 0);
        [self.superview addSubview:_leftTableView];
        
        _leftTableView.alpha = 1.f;
        [UIView animateWithDuration:0.2 animations:^{
            _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth, _height);
